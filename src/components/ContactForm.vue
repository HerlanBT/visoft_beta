...
<form
  ...
  class="w-full max-w-2xl
         bg-text-visoft-gradient/5 dark:bg-[var(--accent)]/20
         backdrop-blur-sm rounded-2xl p-6 shadow-md
         border border-transparent dark:border-[var(--secondary)]/10
         transition-colors"
  aria-label="Formulario de contacto Visoft"
>
  <!-- Header -->
  <div class="mb-4 text-center">
    <h2 class="text-2xl md:text-3xl font-bold text">
      {{ title }}
    </h2>
  ...
  <div :class="compact ? 'space-y-3' : 'grid grid-cols-1 gap-4'">
    <input
      ...
      class="w-full p-3 rounded-lg border border-[var(--primary)]
             bg-[var(--text)] text-[var(--accent)] placeholder:opacity-60 focus:outline-none
  ...
  <div class="mt-5 flex flex-col sm:flex-row items-center gap-3 justify-center">
    <button
      ...
      class="inline-flex items-center gap-3 px-5 py-3 rounded-full font-semibold
             bg-[var(--primary)] text-[var(--surface)] transition-all
